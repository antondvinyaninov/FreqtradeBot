...
После первого старта ожидаются строки `collector started` и `backfill pair=...` для каждой пары. История загружается страницами по 1000 свечей, начиная от уже сохранённой истории и двигаясь назад до заданного периода. Последующие циклы дописывают только новые данные.
